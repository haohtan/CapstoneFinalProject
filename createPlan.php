...
if(isset($_POST['submit'])){
    ...
    $sql = "insert into `plan`(`day_content`,`plan_title`,`start_date`,`end_date`,`destination`,`userID`) values ('$dayContent','$title','$startDate','$endDate','$destination',$userID)";
    if(insertFun($connect,$sql)){
        echo "<script>window.location.href='./plan.php'</script>";
        exit();
    }else{
        echo "<script>alert('Create Fail!');history.back(-1)</script>";
        exit();
    }
            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="title" id="title" class="form-style" placeholder="Plan Title" required>
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="form-group">
                        <label for="">Destination</label>
                        <select name="destination" id="destination" required class="form-style">
                            <?php
                            foreach ($res as $row) {
                ...
                <div class="col-lg-6">
                    <div class="form-group">
                        ...
                        <input type="date" name="startDate" id="startDate" class="form-style" placeholder="Start Date" required>
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="form-group">
                        <label for="">End Date</label>
                        <input type="date" name="endDate" id="endDate" class="form-style" placeholder="End Date" required>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="col-lg-12" style="text-align:right;" >
                    <button type="button"  id ="disappear"  class="button button-primary" onclick="createDate()">Create Date</button>
...
、                </div>
            ...
            <div class="row planDay">
                <div class="col-lg-2 dateButton">
                    <ul class="nav nav-pills mb-3" style="display:flex;flex-direction:column;text-align:center" id="pills-tab" role="tablist">
...
<script>
    ...
    function createDate() {
        ...
        if (startDate.getTime() > endDate.getTime()) {
            alert("The start date cannot earlier than the end date!");
            return false;
        } else {
  
            var dataAmount = (endDate.getTime() - startDate.getTime()) / (1 * 24 * 60 * 60 * 1000);
            if (dataAmount > 7) {
                alert("The Plan date cannot be more than 7 days!");
                return false;
            }
    ...
    function getLocation(text,date) {
        ...
        if (geocoder) {
                    ...
                    success: function(res) {
                        if (res.res.length > 0) {
                            ...
                            for (var i = 0; i < res.res.length; i++) {
                                str = str + "<tr><td>" + res.res[i].content + "</td><td><button class='button button-primary' onclick='selectFavoriteTag(this,\"" + EscapeChar(res.res[i].content) + "\",\"" + id + "\")'>select</button></td></tr>";
                            }
            ...
            function selectFavoriteTag(obj, content,id) {
                ...
                $(obj).parents(".tagInput").find(".tagInput1 ol").append("<li class='selectTagContent'>" + content + "<button onclick='cancelTag(this,\""+EscapeChar(content)+"\",\""+id+"\")' class='deleteTag button button-danger'>Delete</button></li>");
            }
     ...
     for(var i = 0;i<data.length;i++){
           ...
           if(data[i] != content){
            ...
            function selectSuggestionTag(obj, content,id) {
                ...
                $(".h"+id).val(data);
                getLocation(content,id)
                $(obj).parents(".tagInput").find(".tagInput2 ol").append("<li class='selectTagContent'>" + content + "<button onclick='cancelTag(this,\""+EscapeChar(content)+"\",\""+id+"\")' class='deleteTag button button-danger'>Delete</button></li>");
            }
            ...
            function selectResturantTag(obj, content,id) {
                ...
                $(".h"+id).val(data);
                getLocation(content,id)
                $(obj).parents(".tagInput").find(".tagInput3 ol").append("<li class='selectTagContent'>" + content + "<button onclick='cancelTag(this,\""+EscapeChar(content)+"\",\""+id+"\")' class='deleteTag button button-danger'>Delete</button></li>");
            }
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.res.length > 0) {
                            var str = "";
                            for (var i = 0; i < res.res.length; i++) {
                                str = str + "<tr><td>" + res.res[i].content + "</td><td><button class='button button-primary' onclick='selectSuggestionTag(this,\"" + EscapeChar(res.res[i].content) + "\",\"" + id + "\")'>select</button></td></tr>";
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.res.length > 0) {
                            var str = "";
                            for (var i = 0; i < res.res.length; i++) {
                                str = str + "<tr><td>" + res.res[i].content + "</td><td><button class='button button-primary' onclick='selectResturantTag(this,\"" + EscapeChar(res.res[i].content) + "\",\"" + id + "\")'>select</button></td></tr>";
